...
`WD_ROOT=/path/to/checkout/public node run.js --check` points it at another
...
`smoke.js` drives headless Chrome against a local `wrangler dev`
...
on any console error, page error, failed request or 4xx/5xx.
    ...
    npx wrangler dev --port 8788 &                # from the repo root
